...
public class DvlSqlInsertIntoExpression<TParam> : DvlSqlInsertExpression where TParam : ITuple
{
    ...
    public DvlSqlInsertIntoExpression(string tableName, params DvlSqlType[] types) : base(tableName,
        types.Select(t => t.Name ?? throw new ArgumentNullException(nameof(t))).ToArray())
    {
        IsRoot = true;
        DvlSqlTypes = types;
...
public class DvlSqlInsertIntoSelectExpression : DvlSqlInsertExpression
{
    ...
    public DvlSqlInsertIntoSelectExpression(string tableName, IEnumerable<string> columns) : base(tableName,
        columns.ToArray()) =>
        IsRoot = false;
