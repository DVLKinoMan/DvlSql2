...
public static class SqlType
{
    public static DvlSqlType GetDefaultDvlSqlType<TValue>(string name, TValue value) =>
        value switch
        {
            ...
            double doub => Float(name, doub),
            float fl => Float(name,fl),
            long lo => BigInt(name, lo),
            int i => Int(name, i),
    ...
    public static DvlSqlType GetDefaultDvlSqlType(this Type type, string name) =>
        type switch
        {
            ...
            { } t when t == typeof(double) => FloatType(name),
            { } t when t == typeof(float) => FloatType(name),
            { } t when t == typeof(long) => BigIntType(name),
