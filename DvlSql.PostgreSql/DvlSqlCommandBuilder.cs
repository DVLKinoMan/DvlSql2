...
internal class DvlSqlCommandBuilder(StringBuilder command) : ISqlExpressionVisitor
{
    ...
    public void Visit(DvlSqlFromExpression expression)
    {
        switch (expression)
        {
            case DvlSqlFromWithTableExpression { } fromWithTable:
                ...
                // Postgresql does not need no lock
    ...
    public void Visit(DvlSqlFullSelectExpression expression)
    {
        ...
        if (expression.Select.Top != null)
            _command.Append($"{Environment.NewLine}LIMIT {expression.Select.Top}");
    }
    ...
    public void Visit(DvlSqlSkipExpression expression)
    {
        if (expression.FetchNextRows != null)
            _command.Append($"{Environment.NewLine}LIMIT {expression.FetchNextRows} OFFSET {expression.OffsetRows}");
        else _command.Append($"{Environment.NewLine}OFFSET {expression.OffsetRows}");
    }

    public void Visit(DvlSqlTableDeclarationExpression expression)
    {
        _command.Append($"{Environment.NewLine}CREATE TEMP {expression.TableName} TABLE (");
    ...
    {
        _command.Append($"{Environment.NewLine}RETURNING {string.Join(',', expression.Columns)}");
