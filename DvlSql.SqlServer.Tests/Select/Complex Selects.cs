...
public class ComplexSelects
{
    ...
    public void TestMethod1()
    {
        var select = _sql1
            ...
            .Join(AsExp("nbe.BANK_DATA", "B2"), ConstantExpCol("B1.REC_ID") == ConstantExpCol("B2.REC_ID"))
            .Where(
                ConstantExpCol("B1.AMOUNT") < 35000 &
                !InExp("B1.REC_ID", FullSelectExp(SelectTopExp(4, "REC_ID"), FromExp("nbe.BANK_DATA"))) &
                !!!LikeExp("B1.RESTRICT_CODE", "%dd%") &
                ConstantExpCol("B1.ADD_DATE") > new DateTime(2012, 1, 1)
            )
            .GroupBy("B1.AMOUNT")
        ...
        string expectedSelect = Regex.Escape(
            string.Format("SELECT B1.AMOUNT, COUNT(*) AS [CountExp] FROM nbe.BANK_DATA AS [B1] WITH(NOLOCK){0}" +
                          ...
                          "WHERE ((( B1.AMOUNT < @p1 AND B1.REC_ID NOT IN ( SELECT TOP 4 REC_ID FROM nbe.BANK_DATA )) AND B1.RESTRICT_CODE NOT LIKE '%dd%' ) AND B1.ADD_DATE > @p2 ){0}" +
    ...
    public void TestMethod5()
    {
        ...
        string expectedSelect1 = Regex.Escape(string.Format(
            ...
            "INNER JOIN nbe.BANK_DATA AS [B2] ON B1.REC_ID = B2.REC_ID{0}" +
            "WHERE ((( B1.AMOUNT < @p1 AND B1.REC_ID NOT IN ( SELECT TOP 4 REC_ID FROM nbe.BANK_DATA )) AND B1.RESTRICT_CODE NOT LIKE '%dd%' ) AND B1.ADD_DATE > @p2 )",
            Environment.NewLine));
    ...
    private static readonly object[][] ParamsForMethod =
    [
    ...
    [Test]
    [TestCaseSource(nameof(ParamsForMethod))]
    public void TestMethod6(bool onlyOwn, DateTime? startDateTime, DateTime? endDateTime, int userId)
    {
        _ = _sql1.From($"dbo.DocumentsArc X")
            .Where((IsNullExp("@startDate") | ConstantExpCol("X.CreatedAt") >= ConstantExpCol("@startDate")) &
                   (IsNullExp("@endDate") | ConstantExpCol("X.CreatedAt") <= ConstantExpCol("@endDate")) &
                   (onlyOwn
                ...
                Params(
                    Param("@startDate", DateTime(startDateTime ?? DateTime.Now)),
                    Param("@endDate", DateTime(endDateTime ?? DateTime.Now)),
